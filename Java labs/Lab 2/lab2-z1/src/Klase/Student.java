...
public class Student {
    ...
    public Student (String ime, String prezime, Date datumRodjenja, String brojIndeksa, Odsjek odsjek, Integer godinaStudija) throws StudentBuducnostException, PremladStudentException {
        ...
        setOcjene(new ArrayList<Integer>());            //konstruktor koji poziva settere
    }
    //mora se ova sintaksa koristiti kada funkcija baca izuzetak, i pozeljno je navesti koji
    Double prosjek() throws DijeljenjeSNulomException {
        if(ocjene.isEmpty()) throw new DijeljenjeSNulomException("Student nema ni jednu unesenu ocjenu!");
        Double prosjek = 0.0;           //"throw new " pa ime izuzetka
        for(Integer ocjena : ocjene)
            prosjek += ocjena;
        return prosjek/ocjene.size();
    }

    public String to_string(){      //inace obicno se overridea standardna metoda toString()
        try{
            prosjek();
        }
        catch(DijeljenjeSNulomException e){
            System.out.println(e.getMessage());                 //metoda za konverziju u string
        }
    ...
    public void setDatumRodjenja(Date datumRodjenja) throws StudentBuducnostException, PremladStudentException {
        Date today = new Date();                         //bacanje izuzetaka
        if (datumRodjenja.compareTo(today) >= 0)        //moze se datum porediti kako se ocekuje, today je jedan od legalnih izraza
            ...
            throw new PremladStudentException("Klase.Student ne moze biti mladji of 16 godina");
