...
public:
    Kolekcija(bool omoguciDupliranje = true) : _elementi1(nullptr), _elementi2(nullptr), _trenutno(0), _omoguciDupliranje(omoguciDupliranje) {}
    ...
    void AddElement(T1 prvi, T2 drugi){
        ...
        if(!_elementi1 && !_elementi2){
            _elementi1 = new T1[100];
            _elementi2 = new T2[100];
        }

        if(!_omoguciDupliranje){
            for(int i=0; i<*_trenutno; i++){
                if(_elementi1[i] == prvi || _elementi2[i] == drugi)
                    throw exception("Nije dozvoljeno dupliciranje!");
            }
        }
    ...
    Kolekcija InsertAt(int index, T1 prvi, T2 drugi){
        for(int i=(*_trenutno)-2, i>index; i--){
            _elementi1[i+1] = _elementi1[i];
            _elementi2[i+1] = _elementi2[i]; 
        }
        _elementi1[index] = prvi;
        _elementi2[index] = drugi; 
    }
...
char *GetNizKaraktera(const char *c){
    if(!c) return nullptr;

    int vel=std::strlen(c+1);
...
public:
    Komentar(const char* sadrzajKomentara="") {
        ...
        _ocjeneKriterija = new Kolekcija<Kriteriji, int>[4]; 
        _ocjeneKriterija->getElement1(0) = Kriteriji::CISTOCA;      
        _ocjeneKriterija->getElement1(1) = Kriteriji::LOKACIJA;      
        _ocjeneKriterija->getElement1(2) = Kriteriji::UDOBNOST;      
        _ocjeneKriterija->getElement1(3) = Kriteriji::USLUGA;
        for(int i=0; i<4; i++) _ocjeneKriterija->getElement2(i) = -1;      
    }
    ~Komentar() {
    ...
    void AddOcjenuKriterija(Kriteriji k, int oc){
        if(_ocjeneKriterija->getElement2(k) != -1) throw exception("Nije dozvoljeno dupliciranje ocjena!");
        _ocjeneKriterija->getElement2(k) = oc;
    }

    char* GetSadrzajKomentara() { return _sadrzajKomentara; }
    Kolekcija<Kriteriji, int>*  GetOcjeneKriterija() { return _ocjeneKriterija; }

    friend ostream& operator<<(std::ostream &tok, const Komentar &k){
        return tok<<k._sadrzajKomentara<<endl<<"\t"<<*k._ocjeneKriterija;
    }
};


bool ValidirajBrojPasosa(string s) {
    // ^ - start of string
    // [A-Z]{1,2} - one or two uppercase letters
    // [0-9]{3,4} - three or four digits
    // ([- ]?) - optional dash or space
    // [0-9]{2,4} - two to four digits
    // $ - end of string
    std::regex rgx("^([A-Z]{1,2})([0-9]{3,4})([- ]?)([0-9]{2,4})$");
...
public:
    Gost(const char* imePrezime,  string emailAdresa, string brojPasosa) {
        _imePrezime = make_unique<char[]>(*imePrezime);
...
public:
    Rezervacija(Datum& OD, Datum& DO, Gost& gost):_OD(OD), _DO(DO) {
        for(const Gost &g : _gosti) if(g.GetEmail() == gost.GetEmail()) throw domain_error("Gost je vec prijavljen!");

        _gosti.push_back(gost);
    }

    bool AddGost(const Gost &gost){
        for(const Gost &g : _gosti) if(g.GetEmail() == gost.GetEmail()) return false;
        _gosti.push_back(gost);
        return true;
    }

    void SetKomentar(const Komentar &k){
        _komentar = k;
