//TP 2023/2024: Zadaća 5, Zadatak 4
#include <algorithm>
#include <iostream>
#include <cmath>
#include <stdexcept>
...
class PolimorfniSpremnik{
    Spremnik *spr;
    void Test() const {if(!spr) throw std::logic_error("Nespecificiran spremnik");}
public:
    ...
    PolimorfniSpremnik(const PolimorfniSpremnik &p){
        if(!p.spr) spr=nullptr;
        else spr = p.spr->DajKopiju();
    }
    ...
    PolimorfniSpremnik &operator =(const PolimorfniSpremnik &p){
        if(this!=&p){
            Spremnik *pom = nullptr;
            if(p.spr != nullptr) pom = p.spr->DajKopiju();
            delete spr;
            spr=pom;
        }
        return *this;
    ...
    double DajTezinu() {Test(); return spr->DajTezinu();}
    double DajUkupnuTezinu() {Test(); return spr->DajUkupnuTezinu();}
    void Ispisi() {Test(); spr->Ispisi();}
};
...
public:
    ...
    Spremnik* DodajSpremnik(std::shared_ptr<Spremnik>, bool);
...
Spremnik* Skladiste::DodajSpremnik(std::shared_ptr<Spremnik> s, bool moj){      //obicni pokazivac ili? ova cijela metoda moze bit opasna i netacna
    if(moj){
...
int main ()
{
    try{
        Skladiste s;
        Spremnik* sanduk1 = s.DodajSanduk(10.0, "Cigle", {2.0, 3.5, 1.0});
        Spremnik* vreca1 = s.DodajVrecu(5.0, "riza", 25);
        Spremnik* bure1 = s.DodajBure(7.0, "lopate", 1300, 500);
        std::cout << "Najlakši spremnik: " << s.DajNajlaksi().DajSadrzaj() << std::endl;
        std::cout << "Najteži spremnik: " << s.DajNajtezi().DajSadrzaj() << std::endl;
        std::cout<<"Težih od 25kg: "<<s.BrojPreteskih(25)<<std::endl<<std::endl;
        s.BrisiSpremnik(&s.DajNajlaksi());
        s.BrisiSpremnik(&s.DajNajtezi());
        s.IzlistajSkladiste();
        std::cout<<std::endl;

        Sanduk sanduk (10.0, "Cigle", {2.0, 3.5, 1.0});
        Vreca vreca(5.0, "riza", 25);
        Bure bure(7.0, "lopate", 1300, 500);
        PolimorfniSpremnik p1(sanduk);
        p1.Ispisi();
        PolimorfniSpremnik p2(vreca);
        std::cout<<p2.DajTezinu()<<std::endl;
        PolimorfniSpremnik p3(bure);
        std::cout<<p3.DajUkupnuTezinu()<<std::endl;
        p1=p3;
        p3=p2;
        p3.Ispisi();
