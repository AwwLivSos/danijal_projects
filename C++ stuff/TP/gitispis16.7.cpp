...
public:
    ...
    Kolekcija(const Kolekcija &k){
        _trenutno = new int (k.getTrenutno());
    ...
    void AddElement(T1 elem1, T2 elem2){
        if(_trenutno == nullptr) 
            _trenutno = new int(0);
        if(getTrenutno() == max) throw domain_error("Puna kolekcija!");
    ...
    void RemoveAt(int index){
        if(_trenutno == nullptr) return;
        if(index < 0 || index > (*_trenutno)-1) return;
    ...
    T2 &operator[](T1 elem){
        for(int i=0; i<getTrenutno(); i++){
            if(_elementi1[i] == elem) return _elementi2[i];
        }
    }
};
...
/*na odredjeno pitanje kandidat je duzan postaviti vise rjesenja od kojih ce svako biti ocijenjeno*/
class Pitanje {
    char* _sadrzaj;
...
public:
    ...
    friend ostream& operator<< (ostream &COUT, const Pitanje &obj){
        COUT<<"Sadrzaj pitanja je: "<<obj._sadrzaj<<endl;
        COUT<<*obj._ocjeneRjesenja<<endl;
        return COUT;
    }
};
...
public:
    Korisnik(const char* imePrezime, string emailAdresa, string lozinka){
        ...
        _lozinka = "123";//<== na opisani nacin inicijalizovati lozinku
    }
...
proslijedjenim indeksom. nakon uklanjanja vrijednosti onemoguciti pojavu praznog prostora unutar
kolekcije tj. pomjeriti sve elemente koji se nalaze nakon proslijedjene lokacije za
jedno mjesto unazad
 ...
 */
 ...
 dinamickaMemorija("Navedite pristupe za upravljanje dinamicko memorijom."),
